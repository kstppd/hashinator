...
public:
   ...
   Hashinator(const Hashinator<GID, LID>& other) 
      : sizePower(other.sizePower), fill(other.fill),bucket_bank(other.bucket_bank){
         int target_capacity=other.buckets->size();
   ...
   int migrate(split::SplitVector<std::pair<GID, LID>>**src, split::SplitVector<std::pair<GID, LID>>** dst, int newSizePower ){
      ...
      for (auto& e : **src) {
         ...
         for (int i = 0; i < maxBucketOverflow; i++) {
            std::pair<GID, LID>& candidate = (*dst)->operator[]((newHash + i) & bitMask);
   ...
   int rehash2(int newSizePower){

      if (newSizePower > 32) {
         throw std::out_of_range("Hashinator ran into rehashing catastrophe and exceeded 32bit buckets.");
      }

      //Let's start by checking if we have the larger bucket already in our bucket bank
      auto bucket_iter=bucket_bank.begin();
      split::SplitVector<std::pair<GID, LID>>* newBucket_list;
      uint32_t target_capacity= 1<<newSizePower;
      while(bucket_iter!=bucket_bank.end()){
         if (bucket_iter.data()->size()==target_capacity){
            newBucket_list=bucket_iter.data();
            return migrate(&buckets,&newBucket_list,newSizePower);
         }
         ++bucket_iter;
      }
   ...
   /*Here we expand all the buckets except from the one in use*/
   void expand_bucket_bank(int newSizePower){
      ...
      for (int i=0; i < nBlocks; i++){
         ...
         if (bucket_bank[i].size()==buckets->size()){
            continue;
         }
   ...
   void rehash(int newSizePower) {
      ...
      if (nBlocks>1){
         int rehash_status;
         do {
            rehash_status=rehash2(newSizePower);
   ...
   const LID& at(const GID& key) const {
      ...
      // Try to find the matching bucket.
      for (int i = 0; i < maxBucketOverflow; i++) {
         const std::pair<GID, LID>& candidate = buckets->operator[]((hashIndex + i) & bitMask);
         if (candidate.first == key) {
            // Found a match, return that
            return candidate.second;
         }
   ...
   public:
      ...
      iterator& operator++() {
         ...
         while(index < hashtable->buckets->size()){
            if (hashtable->buckets->operator[](index).first != EMPTYBUCKET){
               break;
            }
            index++;
         }
      ...
      bool operator==(iterator other) const {
         return &hashtable->buckets->operator[](index) == &other.hashtable->buckets->operator[](other.index);
      }
      bool operator!=(iterator other) const {
         return &hashtable->buckets->operator[](index) != &other.hashtable->buckets->operator[](other.index);
      }
      std::pair<GID, LID>& operator*() const { return hashtable->buckets->operator[](index); }
      std::pair<GID, LID>* operator->() const { return &hashtable->buckets->operator[](index); }
   ...
   public:
      ...
      const std::pair<GID, LID>& operator*() const { return hashtable->buckets->operator[](index); }
      const std::pair<GID, LID>* operator->() const { return &hashtable->buckets->operator[](index); }
      size_t getIndex() { return index; }
   };
   ...
   iterator begin() {
      for (size_t i = 0; i < buckets->size(); i++) {
         if (buckets->operator[](i).first != EMPTYBUCKET) {
   ...
   iterator erase(iterator keyPos) {
      ...
      if (buckets->operator[](index).first != EMPTYBUCKET) {
         // Decrease fill count
         fill--;

         // Clear the element itself.
         buckets->operator[](index).first = EMPTYBUCKET;
         ...
         for (unsigned int i = 1; i < fill; i++) {
            GID nextBucket = buckets->operator[]((index + i)&bitMask).first;
            ...
            if ((hashIndex&bitMask) != ((index + i)&bitMask)) {
               ...
               if (distance < maxBucketOverflow) {
                  ...
                  LID moveValue = buckets->operator[]((index+i)&bitMask).second;
                  buckets->operator[](targetPos) = std::pair<GID, LID>(nextBucket,moveValue);
                  targetPos = ((index+i)&bitMask);
                  buckets->operator[](targetPos).first = EMPTYBUCKET;
               }
            }
         }
      }
